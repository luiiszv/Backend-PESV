import { findAllFomularios, findFormualrioByID, insertFormulario } from "../services/formulario.service.js";
...
export const registerFormualrio = async ({ body }, res) => {
    try {
        ...
        console.log(error);
        res.status(400).json({ message: "Something went wrong in registerFormualrio", error });
...
export const getAllFormualarios = async (req, res) => {
    try {
        ...
        res.status(400).json({ message: "Something went wrong in filterUserByName", error });
